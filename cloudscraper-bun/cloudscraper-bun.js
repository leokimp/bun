// src/utils.ts
// Use Bun's native atob/btoa — significantly faster than JS implementations
function rot13(str) {
...
async function verifyStreamUrl(url) {
  ...
  // FIX #1: 8s hard timeout — prevents a single slow/504 URL from blocking
  // the entire response for 60+ seconds (was the root cause of Railway timeouts)
  ...
  try {
    ...
    console.log("[VERIFY] \u23F1 Skipped:", reason, "\u2192", url);
    return false;
  }
}
...
// If two identical requests arrive before the first finishes scraping,
